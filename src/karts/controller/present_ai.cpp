...
PresentAI::PresentAI(AbstractKart *kart) 
...
#endif
    setControllerName("Present");
}   // PresentAI

//-----------------------------------------------------------------------------
/** The destructor deletes the shared TrackInfo objects if no more PresentAI
 ...
 *              is searched.
 */
unsigned int PresentAI::getNextSector(unsigned int index)
{
...
void PresentAI::update(float dt)
{
...
//-----------------------------------------------------------------------------
void PresentAI::handleSteering(float dt)
{
    ...
    else
    {
        ...
        for (unsigned int i=0; i < ItemManager::get()->getNumberOfItems(); i++)
        {
            ...
            {
                const Vec3 &pos = item->getXYZ();
                Vec3 dir = pos - me;
                float dist = dir.length2_2d();
        ...
        if ((btAngle(straight_point - me, minpos - me) < M_PI / 8 && mindist < w * 8) ||
            (btAngle(straight_point - me, minpos - me) < M_PI / 4 && 
            btAngle(m_kart->getVelocity(), minpos - me) < M_PI / 6 && mindist < w * 4)) {
            straight_point = minpos;
        }
 ...
 */
void PresentAI::handleItems(const float dt)
{
 ...
 */
void PresentAI::computeNearestKarts()
{
...
//-----------------------------------------------------------------------------
void PresentAI::handleAcceleration( const float dt)
{
...
//-----------------------------------------------------------------------------
void PresentAI::handleRaceStart()
{
    if( m_start_delay <  0.0f )
    {
...
//-----------------------------------------------------------------------------
void PresentAI::handleRescue(const float dt)
{
 ...
 */
void PresentAI::handleNitroAndZipper()
{
    m_controls->m_nitro = false;
...
//-----------------------------------------------------------------------------
void PresentAI::checkCrashes(int steps, const Vec3& pos )
{
 ...
 */
void PresentAI::findNonCrashingPoint(Vec3 *result)
{    
