             ...
             : m_gamepad(NULL)
{
    ...
    {
        m_game_controller = SDL_OpenGamepad(device_id);
        if (!m_game_controller)
            throw std::runtime_error(SDL_GetError());
        m_joystick = SDL_GetGamepadJoystick(m_game_controller);
        if (!m_joystick)
        {
            SDL_CloseGamepad(m_game_controller);
            throw std::runtime_error(SDL_GetError());
        }
    }
    else
    {
        m_joystick = SDL_OpenJoystick(device_id);
        if (!m_joystick)
            throw std::runtime_error(SDL_GetError());
    }

    m_id = SDL_GetJoystickID(m_joystick);
    if (m_id < 0)
    {
        if (m_game_controller)
            SDL_CloseGamepad(m_game_controller);
    ...
    const char* name_cstr = SDL_GetJoystickName(m_joystick);
    if (name_cstr == NULL)
    {
        if (m_game_controller)
            SDL_CloseGamepad(m_game_controller);
    ...
    m_buttons = SDL_GetNumJoystickButtons(m_joystick);
    if (m_buttons < 0)
    {
        if (m_game_controller)
            SDL_CloseGamepad(m_game_controller);
    ...
    m_axes = SDL_GetNumJoystickAxes(m_joystick);
    if (m_axes < 0)
    {
        if (m_game_controller)
            SDL_CloseGamepad(m_game_controller);
    ...
    m_hats = SDL_GetNumJoystickHats(m_joystick);
    if (m_hats < 0)
    {
        if (m_game_controller)
            SDL_CloseGamepad(m_game_controller);
    ...
    if (m_game_controller && SDL_GetGamepadName(m_game_controller))
    {
        Log::info("SDLController", "%s uses game controller mapping %s.",
            name.c_str(), SDL_GetGamepadName(m_game_controller));
    }
    ...
    for (int i = 0; i < m_axes; i++)
        m_prev_axes[i] = SDL_GetJoystickAxis(m_joystick, i);
    ...
    if (m_game_controller)
    {
        char* mapping = SDL_GetGamepadMapping(m_game_controller);
        if (mapping)
        {
...
{
    Log::info("SDLController", "%s unplugged.", SDL_GetJoystickName(m_joystick));
    if (m_game_controller)
        SDL_CloseGamepad(m_game_controller);
    else
        SDL_CloseJoystick(m_joystick);
...
{
// replace with SDL_GetJoystickPowerInfo
...
//        {
//            core::stringw msg =
//                _("%s has low battery level.", SDL_GetJoystickName(m_joystick));
...
void SDLController::doRumble(float strength_low, float strength_high, uint32_t duration_ms)
{
    if (m_haptic)
    {
        SDL_PlayHapticRumble(m_haptic, (strength_low + strength_high) / 2, duration_ms);
    }
    else
    {
        ...
        SDL_RumbleGamepad(m_game_controller, scaled_low, scaled_high, duration_ms);
#endif
    }
}
...
{
    m_auto_center = state;
    SDL_SetHapticAutocenter(m_haptic, m_auto_center);
}
