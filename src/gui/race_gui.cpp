...
void RaceGUI::UpdateKeyboardMappings()
{
    ...
    const int NUM = world->raceSetup.getNumPlayers();
    for(int i=0; i < NUM; i++)
    {
...
void RaceGUI::inputKeyboard(int key, int pressed)
{
    ...
    case 0x12: // TODO: Which key is that?
        if(world->raceSetup.getNumPlayers()==1)
        {   // ctrl-r
...
void RaceGUI::drawFPS ()
{
    ...
    widgetSet->drawDropShadowTextRace (m_fps_string, 48, 0, config->height-50, 255, 255, 255 ) ;
}   // drawFPS
...
void RaceGUI::drawGameOverText (const float dt)
{
    ...
    {
        char s[255];
        sprintf(s,"YOU FINISHED %s",m_pos_string[finishing_position]);
...
#define ICON_PLAYER_WIDHT 50
    ...
    for(int i = 0; i < world->getNumKarts() ; i++)
    {
        ...
        {  // Display for 5 seconds
            float timeBehind;
            timeBehind = (lap==m_lap_leader ? kart->getTimeAtLap() : world->clock)
                         - m_time_of_leader;
