//  $Id: callback.cpp 796 2006-09-27 07:06:34Z hiker $
//
...
// -----------------------------------------------------------------------------
void Callback::parseData(char *data)
{
  char *s = data;
...
// -----------------------------------------------------------------------------
void Callback::update(float dt)
{
  sgCoord add;
 
 
  float timer = world->clock + m_phase ;
 
  if ( m_cycle != 0.0 && m_mode != MODE_FORWARD ) {
    if ( m_mode == MODE_SHUTTLE ) {
      float ctimer = fmod ( timer, m_cycle ) ;
