                                 ...
                                 PositionClass position_class)
{
       ...
       position_class!=POSITION_SOCCER_MODE)
    {
        ...
        if(w_inf=="w" || w_sup=="w")
        {
            ...
            Log::error("PowerupManager","Incorrect reference points in powerup.xml."
       ...
       weight_list_sup.size()!=2*(int)POWERUP_LAST)
    {
        Log::error("PowerupManager",
                   ...
                   class_name.c_str());
        Log::error("PowerupManager",
                   "%d and %d instead of twice %d - probabilities will be incorrect.",
                   (int)weight_list_inf.size(), (int)weight_list_sup.size(),
                   (int)POWERUP_LAST);
        return;
    }
                                                             ...
                                                             int random_number)
{
   ...
   int random_item = (random_number/RAND_CLASS_RANGE)%m_powerups_for_reference_pos[pos_class].size();
