                ...
                m_draw_call_type(type)
{
    ...
    m_data_layout_pbr = VK_NULL_HANDLE;
    m_descriptor_pool = VK_NULL_HANDLE;
    m_descriptor_pool_pbr = VK_NULL_HANDLE;
...
GEVulkanDrawCall::~GEVulkanDrawCall()
{
    ...
    if (m_data_layout != VK_NULL_HANDLE)
    {
        ...
        vkDestroyDescriptorSetLayout(vk->getDevice(), m_data_layout_pbr, NULL);
        vkDestroyDescriptorPool(vk->getDevice(), m_descriptor_pool, NULL);
        vkDestroyDescriptorPool(vk->getDevice(), m_descriptor_pool_pbr, NULL);
        for (auto& p : m_graphics_pipelines)
            vkDestroyPipeline(vk->getDevice(), p.second.first, NULL);
        for (auto& p : m_depth_only_pipelines)
            vkDestroyPipeline(vk->getDevice(), p.second.first, NULL);
...
start:
    ...
    {
        if (it.second.second.m_push_constants_func)
        {
            std::pair<uint32_t, void*> constant;
            it.second.second.m_push_constants_func(&constant.first, &constant.second);
            m_push_constants[it.first] = constant;
        }
    }
    ...
    for (auto& p : visible_nodes)
    {
        ...
        for (auto& q : p.second)
        {
            ...
            if (pipeline_map.find(cur_shader) ==
                pipeline_map.end())
                continue;
            InstanceKey key;
            ...
            const PipelineSettings& settings =
                pipeline_map[cur_shader].second;
            for (auto& r : q.second)
            {
                irr::scene::ISceneNode* node = r.first;
                if (r.second == BILLBOARD_NODE || r.second == PARTICLE_NODE)
    ...
    else
    {
        ...
        if (skinning_data_padded_size + (object_data_padded_size * 2) >
            m_sbo_data->getSize())
        {
            min_size = skinning_data_padded_size + (object_data_padded_size * 2);
            goto start;
        }
    }
}   // generate
                                                   ...
                                                   GEVulkanSunSceneNode* sun)
{
    ...
    case GVDCT_SHADOW_NEAR:
        m_culling_tool->init(sun, GVSCC_NEAR);
        break;
    case GVDCT_SHADOW_MIDDLE:
        m_culling_tool->init(sun, GVSCC_MIDDLE);
        break;
    case GVDCT_SHADOW_FAR:
        m_culling_tool->init(sun, GVSCC_FAR);
        break;
    default:
        break;
    }
...
void GEVulkanDrawCall::createAllPipelines(GEVulkanDriver* vk)
{
    ...
    settings.m_depth_only_fragment_shader = "solid_depth.frag";
                                      ...
                                      const PipelineSettings& settings)
{
    ...
    {
    case GVDCT_FORWARD:
        create_pipeline_for_depth = getGEConfig()->m_pbr;
...
void GEVulkanDrawCall::createVulkanData()
{
    ...
    VkDescriptorSetLayoutBinding cluster_xz_layout_binding = {};
    cluster_xz_layout_binding.binding = 3;
    cluster_xz_layout_binding.descriptorCount = 1;
    cluster_xz_layout_binding.descriptorType =
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
    cluster_xz_layout_binding.pImmutableSamplers = NULL;
    cluster_xz_layout_binding.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    VkDescriptorSetLayoutBinding cluster_yz_layout_binding = {};
    cluster_yz_layout_binding.binding = 4;
    cluster_yz_layout_binding.descriptorCount = 1;
    cluster_yz_layout_binding.descriptorType =
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
    cluster_yz_layout_binding.pImmutableSamplers = NULL;
    cluster_yz_layout_binding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
                                         ...
                                         VkCommandBuffer custom_cmd)
{
    ...
    case GVDCT_FORWARD:
        ...
        break;
    case GVDCT_SHADOW_NEAR:
        data_uploading.emplace_back((void*)m_sun->getShadowCameraUBOData(GVSCC_NEAR), sizeof(GEVulkanCameraUBO));
        break;
    case GVDCT_SHADOW_MIDDLE:
        data_uploading.emplace_back((void*)m_sun->getShadowCameraUBOData(GVSCC_MIDDLE), sizeof(GEVulkanCameraUBO));
        break;
    case GVDCT_SHADOW_FAR:
        data_uploading.emplace_back((void*)m_sun->getShadowCameraUBOData(GVSCC_FAR), sizeof(GEVulkanCameraUBO));
        break;
    default:
        break;
    }
...
void GEVulkanDrawCall::render(GEVulkanDriver* vk, VkCommandBuffer custom_cmd)
{
    ...
    case GVDCT_SHADOW_FAR:
        vp.x = 0;
        vp.y = 0;
        vp.width = GEVulkanSunSceneNode::getShadowMapSize();
        vp.height = GEVulkanSunSceneNode::getShadowMapSize();
        vp.minDepth = 0;
        vp.maxDepth = 1.0f;
        break;
    default:
    ...
    std::vector<uint32_t> dynamic_offsets_pbr =
    {
        0u
    };
...
start:
    ...
    else
    {
        ...
        bool rebind_base_vertex = true;
        bool current_pipeline_exists = bindPipeline(cmd, cur_pipeline, is_depth_only);
        ...
        for (unsigned i = 0; i < m_cmds.size(); i++)
        {
            ...
            {
                cur_pipeline = m_cmds[i].m_shader;
                bool current_pipeline_exists = 
                    bindPipeline(cmd, cur_pipeline, is_depth_only);
                if (!current_pipeline_exists)
                {
                    continue;
                }
...
void GEVulkanDrawCall::updateDataDescriptorSets(GEVulkanDriver* vk)
{
    ...
    for (unsigned i = 0; i < m_data_descriptor_sets.size(); i++)
    {
        ...
        sbo_info_skinning.buffer = m_sbo_data->getHostBuffer()[i];
        ...
        sbo_info_material.buffer = m_sbo_data->getHostBuffer()[i];
        sbo_info_material.offset = m_skinning_data_padded_size + m_object_data_padded_size;
