...
{
private:
    GEVulkanSunSceneNode *m_active_sun;

    std::map<GEVulkanCameraSceneNode*, std::unique_ptr<GEVulkanDrawCall> >
        m_draw_calls;

    std::map<GEVulkanCameraSceneNode*, std::vector<std::unique_ptr<GEVulkanDrawCall> > >
        m_shadow_draw_calls;
...
public:
    ...
    // ------------------------------------------------------------------------
    //! Sets the active sun. The previous active sun will be deactivated.
    //! \param sun: The new sun which should be active.
    virtual void setActiveSun(GEVulkanSunSceneNode* sun) { m_active_sun = sun; }
     // ------------------------------------------------------------------------
    //! Returns the current active sun.
    //! \return The active sun is returned. Note that this can be NULL, if there
    //! was no sun created yet.
    virtual GEVulkanSunSceneNode* getActiveSun() const { return m_active_sun; }
    // ------------------------------------------------------------------------
    virtual void clear();
    // ------------------------------------------------------------------------
